...
public class BattleWalker extends OverlayWalker {
    /**
     * Note that AP cost has a different meaning depending on context. For battle is
     * literal AP, for world and dungeons is chance of encounter, unrelated to time.
     *
     * @author alex
     */
	...
	public BattleStep(Point p, final float totalcost) {
	    super(p);
	    this.engaged = BattleWalker.this.engaged || checkengaged(p);
	    this.apcost = getcost();
	    this.totalcost = totalcost + this.apcost;
	    color = this.engaged || this.totalcost >= .5 ? Color.RED : Color.GREEN;
	...
	this.state = state;
	this.engaged = checkengaged(from);
    }

    @Override
    protected Point step(Point step, LinkedList<Point> previous) {
	float totalcost = previous.isEmpty() ? BattleScreen.partialmove : ((BattleStep) previous.getLast()).totalcost;
	...
	if (state.map[step.x][step.y].blocked && (current.source.fly == 0 || !Javelin.app.fight.map.flying)) {
	    return false;
	}
	boolean istarget = step.equals(to);
	if (this.engaged && (!previous.isEmpty() || !istarget)) {
	    return false;
	}
	if (step.totalcost >= 1 || state.getmeld(step.x, step.y) != null) {
	    return false;
	}
	if (state.getcombatant(step.x, step.y) != null) {
	    return false;
	}
	Meld m = state.getmeld(step.x, step.y);
	if (m != null) {
	    return istarget && current.ap >= m.meldsat;
	}
	try {
	    BattleTile t = (BattleTile) BattleScreen.active.mappanel.tiles[p.x][p.y];
	    return ((BattlePanel) BattleScreen.active.mappanel).daylight || (t.discovered && !t.shrouded);
