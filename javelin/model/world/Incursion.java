...
public class Incursion extends Actor{
  ...
  /** @see #getel() */
  public Combatants squad=new Combatants();
  ...
  void move(){
    ...
    var newx=x+determinemove(x,target.x);
    var newy=y+determinemove(y,target.y);
    if(Terrain.get(newx,newy).equals(Terrain.WATER)){
      displace();
      return;
    }
    var arrived=World.get(newx,newy,World.getactors());
    x=newx;
    y=newy;
    place();
  ...
  public void attack(Actor target){
    var status=target.destroy(this);
    if(status==null) return;
    if(status) target.remove();
    else remove();
  }
  ...
  void fightdefenders(Actor target){
    if(target instanceof Squad
        ||target instanceof Location&&((Location)target).ishostile())
      return;
    var d=target.getdistrict();
    if(d==null||d.town.ishostile()) return;
    var defenders=d.getsquads();
  ...
  public String describe(){
    ...
    var description=this.description+" ("+Difficulty.describe(squad)+" fight)";
    if(enemies.isEmpty()) description+=".";
    else description+=":\n\n"+enemies;
    return description;
  }
  ...
  public boolean fight(List<Combatant> defenders){
    var me=getel();
    var them=ChallengeCalculator.calculateel(defenders);
    var win=fight(me,them);
    if(win) damage(squad,VICTORYCHANCES.get(me,them));
    else damage(defenders,VICTORYCHANCES.get(them,me));
    return win;
  }
  ...
  void damage(List<Combatant> survivors,int chance){
    var totalcr=0;
    for(Combatant c:survivors) totalcr+=c.source.cr;
    var damage=totalcr*(1-chance/10f);
    var wounded=new LinkedList<>(survivors);
    Collections.sort(wounded,CombatantByCr.SINGLETON);
    while(damage>0&&survivors.size()>1){
      var dead=wounded.pop();
  ...
  public static ArrayList<Incursion> getall(){
    var all=new ArrayList<Incursion>();
