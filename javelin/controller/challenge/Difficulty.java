...
public class Difficulty{
  ...
  static{
    DISTRIBUTION.add(-6);
    DISTRIBUTION.add(-5);
    for(var i=0;i<10;i++) DISTRIBUTION.add(-4);
    DISTRIBUTION.add(-3);
    DISTRIBUTION.add(-2);
    DISTRIBUTION.add(-1);
    DISTRIBUTION.add(+0);
   ...
   */
  public static String describe(List<Combatant> squad,
  ...
  /** @return As {@link #describe(List, List)} but rasterizes each side. */
  public static String describemany(List<Combatants> attacking,
      List<Combatants> defending){
    return describe(attacking.stream().flatMap(Combatants::stream).toList(),
        defending.stream().flatMap(Combatants::stream).toList());
  }
}
