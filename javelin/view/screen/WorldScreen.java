...
public class WorldScreen extends BattleScreen {
     ...
     * small amount of screen-space. If the World is small enough to fit a screen we
     * can't have the player walking around too freely or he will be able to reach
     ...
     *
     * Per D&D rules a well-equipped party should be able to withstand 4 encounters
     * in a row before needing to rest. This means that the player can take
     * (statistically speaking) 6 steps away from a Town and still come back to rest
     * properly. This sounds quite fine, even quite liberal, considering how small
     * the map is and that the player can build roads, rent {@link Transport}
     * vehicles, etc - which will further reduce the encounter rate by increasing
     * player speed.
     *
     ...
     * current {@link Monster} selection allows a novice player to select a 15-feet
     * moving unit makes this hard to circumvent.
     */
    ...
    /**
     * @return One line of text containing unit name and status information (health,
     *         poison, etc).
     */
     ...
     * goal back then was to allow you to reach a new {@link Town} after an average
     * of 4 fights (100% resource spending). Now {@link Fortification}s and hostile
     * Towns change everything. The ultimate goal still is to make every move
     * strategically meaningful (no no-brainers) while still keeping the world scre
     * en smal enough to fit in just a few screens worth of size.
     *
     * @param hoursellapsed
	...
	if (World.scenario.worldencounters && (Squad.active.transport == null || Squad.active.transport.battle())
	...
	Set<Hazard> hazards = Squad.active.getdistrict() == null ? Terrain.current().gethazards(special)
	...
	return World.validatecoordinate(p.x, p.y) && getcurrentscreen().gettiles()[p.x][p.y].discovered;
    }
