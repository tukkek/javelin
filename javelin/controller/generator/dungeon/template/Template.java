...
public abstract class Template implements Cloneable,DungeonArea{
	...
	static final ArrayList<Mutator> ROTATORS=new ArrayList<>(
			Arrays.asList(new Mutator[]{Rotate.INSTANCE,HorizontalMirror.INSTANCE,
					VerticalMirror.INSTANCE}));
	static final int FREEMUTATORS;

	static{
		int freemutators=0;
		for(Mutator m:MUTATORS)
			if(m.chance==null) freemutators+=1;
	...
	public ArrayList<Point> find(char tile){
		...
		for(int x=0;x<width;x++)
			for(int y=0;y<height;y++)
				if(tiles[x][y]==tile) found.add(new Point(x,y));
		return found;
	}
	...
	protected boolean validate() throws GaveUp{
		...
		final HashSet<Point> free=new HashSet<>(width*height);
		walk(doors.get(0),free);
		for(Point door:doors)
			if(!free.contains(door)) return false;
		iterate(t->{
	...
	public Point getdoor(Direction d){
		for(Point door:getdoors())
			if(inborder(door.x,door.y)==d) return door;
		return null;
	}
