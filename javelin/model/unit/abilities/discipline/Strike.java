...
public abstract class Strike extends Maneuver {
	class AttackSet extends ArrayList<Attack> {
		public void addattack(Attack a) {
			...
			if (previous == -1) {
				add(a);
			} else if (a.bonus > get(previous).bonus) {
				set(previous, a);
			}
		}
	}
	 ...
	 */
	abstract public void preattacks(Combatant current, Combatant target,
			Attack a, BattleState s);
	 ...
	 */
	abstract public void postattacks(Combatant current, Combatant target,
			Attack a, BattleState s);
	 ...
	 * saving throws, etc).
	 * 
	 * This is called before {@link Attack#effect} is taken into effect (if it
	 * is at all).
	 * 
	 * @param dc
