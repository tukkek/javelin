...
public abstract class Maneuver
	...
	/**
	 * Delegate to {@link Action#perform(Combatant)}. The implementations need not
	 * worry about handling the {@link Combatant#ready(Maneuver)} or
	 * {@link Maneuver#spend()}.
	 *
	 * @see ExecuteManeuver
	 */
	...
	/**
	 * A delegate for {@link AiAction#getoutcomes(Combatant, BattleState)}. By the
	 * time this is called, all parameters have already been properly cloned. The
	 * implementations need not worry about handling the
	...
	static public int getinitiationmodifier(Combatant c){
		...
		modifier+=Monster
				.getbonus(Math.max(m.intelligence,Math.max(m.wisdom,m.charisma)));
	...
	public int compareTo(Maneuver o){
		if(level!=o.level) return Integer.compare(o.level,level);
