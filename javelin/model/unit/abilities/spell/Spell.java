...
public abstract class Spell extends Upgrade
  ...
  static public void setup(){
    for(var k:Kit.KITS) for(Upgrade u:k.getupgrades()) if(u instanceof Spell s){
   ...
   * @param crp Challenge rating factor.
   */
  public Spell(String name,int levelp,float crp){
    super(name);
   ...
   *   <code>null</code>, for example, if this spell is being triggered by an
   *   {@link Attack} effect.
   ...
   *   {@link Integer#MAX_VALUE} in case of an automatic success; or the minimum
   *   number needed to roll on a d20 for a successful saving throw. With the
   *   exceptions above numbers will be approximated into the range: ]2,19] to
   *   allow the ensuing roll of 1 to always be an automatic miss and 20 an
   *   automatic hit.
   *
  ...
  /**
   * Helper method for {@link #filter(Combatant, List, BattleState)}.
   *
  ...
  static public void targetself(final Combatant combatant,
    ...
    targets.retainAll(List.of(combatant));
  }

  /**
   * @return {@link Monster#getbonus(int)} for the best of
   ...
   *   to succesfully save against this spell or {@link Integer#MIN_VALUE} If
   *   the monster is immune to this type of save.
   *
   * @see Monster#ref
  ...
  public boolean castpeacefully(Combatant c){
    ...
    var s=Squad.active;
    if(castonallies){
      var targeti=Javelin.choose("Cast on which ally?",s.members,true,false);
   ...
   *   not return less than 1 so that the effect will be in place for at least
   *   some amount of time.
   * @see Monster#ref
   * @see Monster#fort
   * @see Monster#getwill()
