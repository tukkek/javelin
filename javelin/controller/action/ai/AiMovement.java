...
public class AiMovement extends Action implements AiAction {
    /**
     * Target value for number of outcome nodes.We want to limit the nu,ber of node
     * outcomes so that the AI doesn't become to slow.
     */
	...
	public LongMove(Combatant c, BattleStep s, List<Point> steps, BattleWalker mover, Meld m, Node n) {
	    super(n, 1, getmessage(c, m, s), m == null ? Javelin.Delay.WAIT : Javelin.Delay.BLOCK);
	...
	ArrayList<List<ChanceNode>> outcomes = new ArrayList<List<ChanceNode>>(Math.min(destinations.size(), MOVES));
    ...
    ArrayList<Point> getpointsofinterest(Combatant c, BattleState s, HashSet<Point> destinations) {
	final ArrayList<Combatant> enemies = s.getteam(c) == s.blueTeam ? s.redTeam : s.blueTeam;
