...
public class Retire extends WorldAction{
  ...
  static List<String> CONFIRM=Arrays.asList("Yes, I am sure.",
      "Not really, keep unit.");
  ...
  public void perform(WorldScreen screen){
    ...
    var mercenaries=Squad.active.getmercenaries();
    var dismissmercenaries=Integer.MIN_VALUE;
    if(mercenaries.size()>1){
      dismissmercenaries=squad.indexOf(mercenaries.get(0));
      members.add(dismissmercenaries,ALLMERCENARIES);
    }
    var prompt="Which squad member do you want to retire?";
    var choice=Javelin.choose(prompt,members,true,false);
  ...
  void dismissmercenaries(List<Combatant> mercenaries){
    if(confirm("Do you want to retire all of your mercenaries?"))
      for(Combatant c:mercenaries) c.dismiss(Squad.active);
  }

  void dismiss(Combatant chosen){
    var dailyupkeep=chosen.source.eat();
    if(chosen.mercenary) dailyupkeep+=chosen.pay();
    final var formattedcost=Javelin.format(Math.round(dailyupkeep));
