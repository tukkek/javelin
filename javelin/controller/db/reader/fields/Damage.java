...
public class Damage extends FieldReader{
  static String[] ELEMENTALDAMAGE={"fire","cold","electricity","acid","energy",
      "positive"};
  ...
  public void read(String value){
    ...
    for(final String blackList:new String[]{"transformation","permanent",
        "corporeal instability","mummy rot","disease","strand","disruption",
        "slaying","entangle","stun","implant","energy drain","wounding",
        "armor damage"})
    ...
    if(nocrit) MonsterReader.log(reader.monster+": no critical damage data",
        "monsters.log");
  }

  void parseattacks(String value,final ArrayList<Attack> attacks){
    for(String attack:value.replace(",",";").split(";"))
      if(!parseattacks(attacks,attack)) reader.errorhandler.setInvalid(
          "Attack not found: "+attack+" ("+reader.monster+") #damage");
  }

  boolean parseattacks(final ArrayList<Attack> attacks,final String attackp){
    var attack=attackp.replace(" or","").replace("or ","")
        .replaceAll("  "," ").trim();
    Spell effect=null;
    for(DamageEffect e:DamageEffect.EFFECTS) if(attack.contains(e.name)){
    ...
    var found=false;
    for(final Attack a:attacks) if(parseattack(attack,a)){
      if(effect!=null) a.seteffect(effect);
      found=true;
    }
    return found;
  }
  ...
  void setdamage(String damage,final Attack a){
    try{
      ...
      a.damage=parsedamage(
          damage.substring(cutname(damage)+1,damage.lastIndexOf(' ')).trim());
      a.getAverageDamageNoBonus();
      final var crit=damage
          .substring(damage.indexOf('(')+1,damage.indexOf(')')).split("/");
  ...
  String parseelementaldamage(String damage,final Attack a){
    for(String element:ELEMENTALDAMAGE) if(damage.contains(element)){
      a.energy=true;
  ...
  int cutname(final String attack){
    for(var i=attack.lastIndexOf(' ')-1;i>=0;i--)
      if(attack.charAt(i)==' ') return i;
    return -1;
  }

  static private int[] parsedamage(final String damage){
    final var beforeD=damage.replaceAll(" ","").split("d");
    final var afterD=beforeD[1];
    final String sides;
    final String bonus;
    if(afterD.contains("+")){
      final var split=afterD.split("\\+");
      sides=split[0];
      bonus=split[1];
    ...
    return new int[]{Integer.parseInt(beforeD[0]),Integer.parseInt(sides),
        Integer.parseInt(bonus)};
  }

  ArrayList<Attack> getallattacks(){
    final var attacktypes=new ArrayList<List<AttackSequence>>();
    ...
    for(final List<AttackSequence> attacktype:attacktypes)
      for(final List<Attack> thisattack:attacktype) attacks.addAll(thisattack);
    return attacks;
  }
}
