...
public class Squad extends Actor implements Cloneable,Iterable<Combatant>{
  ...
  public void join(final Squad s){
    ...
    for(final Combatant c:s.members) equipment.put(c,s.equipment.get(c));
  ...
  public float eat(){
    var sum=0F;
    for(final Combatant c:members) sum+=c.source.eat();
    return sum;
  }
  ...
  public boolean fly(){
    ...
    for(Combatant c:members) if(c.source.fly==0) return false;
    return true;
  }
  ...
  public static int perceive(boolean flyingbonus,boolean weatherpenalty,
    ...
    for(Combatant c:squad){
      var roll=10+c.perceive(flyingbonus,weatherpenalty,periodbonus);
  ...
  public String scout(List<Combatant> opponents,Actor target){
    ...
    var spot=perceive(true,true,true);
  ...
  public boolean hide(List<Combatant> foes){
    ...
    var outside=Dungeon.active==null;
    var flying=outside&&!Terrain.current().equals(Terrain.FOREST);
    var listenroll=Squad.active.perceive(flying,outside,outside);
    var listen=false;
    for(Combatant foe:foes) if(listenroll>=foe.taketen(Skill.STEALTH)){
      listen=true;
      break;
    }
    if(!listen) return false; // doesn't hear them coming
    var hideroll=getworst(Skill.STEALTH).roll(Skill.STEALTH);
  ...
  public boolean bribe(List<Combatant> foes){
    var intelligent=false;
    for(Combatant c:foes) if(c.source.think(-1)){
      intelligent=true;
      break;
    }
   ...
   *   amount covered in an hour but the correspoinding movement per day is less
   *   since it has to account for sleep, resting, etc.
   */
  public int speed(Terrain t,int x,int y){
    var snow=t.describeweather()==Terrain.SNOWING?2:1;
    if(transport!=null){
      var transportspeed=transport.getspeed(members)/snow;
      return transport.flies?transportspeed:t.speed(transportspeed,x,y);
    }
    var speed=Integer.MAX_VALUE;
    var allfly=true;
  ...
  public String describe(){
    var members="";
    for(Combatant c:this.members) members+=c+", ";
  ...
  public static ArrayList<Squad> getsquads(){
    var actors=World.getall(Squad.class);
    var squads=new ArrayList<Squad>(actors.size());
    for(Actor a:actors) squads.add((Squad)a);
    return squads;
  }
  ...
  public boolean skipcombat(int diffifculty){
    ...
    while(input!='\n'&&input!='s'){
      Javelin.app.switchScreen(BattleScreen.active);
      final var difficulty=Difficulty.describe(diffifculty);
      final var prompt="Do you want to skip this "+difficulty+" battle?\n\n" //
  ...
  public void seesurroudings(){
    var onairship=Transport.AIRSHIP.equals(transport);
    var vision=perceive(!onairship,true,true);
    if(onairship) vision+=+4;
    else vision+=Terrain.get(x,y).visionbonus;
    seesurroundings(vision);
  }

  public static void updatevision(){
    for(Squad s:Squad.getsquads()) s.seesurroudings();
  }
  ...
  public String wastegold(float resourcesused){
    var spent=Math.round(Squad.active.gold*resourcesused);
    if(spent<=0) return "";
  ...
  public int getupkeep(){
    var upkeep=0;
    for(Combatant c:members) if(c.mercenary) upkeep+=c.pay();
    return upkeep;
  }
  ...
  public int sumxp(){
    ...
    for(Combatant c:Squad.active.members) sum=sum.add(c.xp);
    return Math.round(sum.floatValue()*100);
  }

  /**
   * @return All ready-to-use available spells and item to heal
   *   {@link #members}. Sources of healing that can be used multiple times will
   *   be repeated this many times in the list.
   * @see #quickheal()
   */
  public List<Healing> canheal(){
    var available=new ArrayList<Healing>();
    for(var c:members) spell:for(var s:c.spells)
      for(var target:members) if(!s.exhausted()&&s.canheal(target)){
        var nheals=s.getheals();
        for(var i=0;i<nheals;i++) available.add(s);
        continue spell;
      }
    for(var user:members) items:for(var i:equipment.get(user))
      if(i.canuse(user)==null) for(var target:members) if(i.canheal(target)){
        var nheals=i.getheals();
        for(var j=0;j<nheals;j++) available.add(i);
        continue items;
      }
    return available;
  }
  ...
  public void quickheal(){
    ...
    healing:for(var h:healing){
      ...
      for(var target:members) if(h.canheal(target)){
        h.heal(target);
        continue healing;
      }
    }
   ...
   *   that are injured or worse won't be able to heal others. If nobody can
   *   heal, returns {@link Integer#MIN_VALUE}.
   *
   * @see Combatant#getstatus()
   */
  public int heal(){
    var heal=Integer.MIN_VALUE;
    for(Combatant c:Squad.active.members)
  ...
  public int roll(Skill s){
    var highest=Integer.MIN_VALUE;
    for(var member:members) highest=Math.max(highest,member.roll(s));
    return highest;
  }
  ...
  public void delay(long hours){
    if(hours<=0) return;
    time+=hours;
    for(var c:new ArrayList<>(members)) c.terminateconditions((int)hours);
