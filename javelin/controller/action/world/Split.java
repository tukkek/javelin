...
public class Split extends WorldAction {
	...
	public void perform(final WorldScreen screen) {
		...
		while (input != '\n') {
			...
			String text = "";
			for (int i = 0; i < Math.max(oldcolumn.size(), newcolumn.size()); i++) {
			...
			Combatant swap;
			try {
				swap = indexreference.get(Integer.parseInt(Character
						.toString(input)) - 1);
		...
		while (input != '\n') {
			clear();
			input = input("How much gold do you want to transfer to the new squad? Use the + and - keys to change and ENTER to confirm.\n"
					+ gold);
			if (input == '+') {
				gold += increment;
		...
		int x = Squad.active.x - 1;
		int y = Squad.active.y - 1;
		placement: for (; x <= Squad.active.x + 1; x++) {
			for (; y <= Squad.active.y + 1; y++) {
				if (!WorldMap.isoccupied(x, y, false)) {
					break placement;
				}
			}
		}
		final Squad s = new Squad(x, y, Squad.active.hourselapsed
				+ WorldMove.TIMECOST);
		s.members = newsquad;
		s.gold = gold;
		...
		for (final Combatant m : newsquad) {
			...
			final List<Item> items = Squad.active.equipment.get(name);
			Squad.active.equipment.remove(name);
			s.equipment.put(name, items);
		}
	}
