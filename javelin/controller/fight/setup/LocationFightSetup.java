...
public class LocationFightSetup extends BattleSetup{
	static final ArrayList<Point> DELTAS=new ArrayList<>();
	static{
		for(int x=-1;x<=+1;x++)
			for(int y=-1;y<=+1;y++)
	...
	protected void place(List<Combatant> team,List<Point> startingarea){
		...
		teamplacement:for(Combatant c:team){
			...
			for(Point p:new ArrayList<>(startingarea)){
	...
	protected boolean place(Combatant c,int x,int y){
		if(map.map[x][y].blocked||Fight.state.getcombatant(x,y)!=null) return false;
		c.location[0]=x;
		c.location[1]=y;
		return true;
	}
