...
public abstract class AbstractAttack extends Action implements AiAction {
	...
	class AttackNode extends ChanceNode {
		public AttackNode(Node n, float chance, String action, Delay delay, Combatant target) {
	...
	public List<ChanceNode> attack(final BattleState gameState, final Combatant current, final Combatant target,
			CurrentAttack attacks, int bonus) {
		return attack(gameState, current, target, attacks.getnext(), bonus, getdamagebonus(current, target),
				AbstractAttack.calculateattackap(getattacks(current).get(attacks.sequenceindex)));
	}
	...
	public List<ChanceNode> attack(final BattleState gameState, Combatant current, final Combatant target,
			final Attack attack, int bonus, int damagebonus, final float ap) {
		current = gameState.clone(current);
		current.ap += ap;
		final ArrayList<ChanceNode> nodes = new ArrayList<ChanceNode>();
		for (final DamageChance dc : dealattack(gameState, current, target, bonus, attack)) {
			if (dc.damage > 0) {
	...
	List<DamageChance> dealattack(final BattleState gameState, final Combatant current, final Combatant target,
			int bonus, final Attack a) {
		bonus += a.bonus;
		if (a.touch) {
		...
		final float confirmchance = target.source.immunitytocritical ? 0 : threatchance * hitchance;
		final float savechance = a.effect == null ? 0 : CastSpell.savechance(current, target, a.effect);
		final float nosavechance = 1 - savechance;
		chances.add(new DamageChance(misschance, 0, false, null));
		AbstractAttack.hit(a, chances, (hitchance - confirmchance) * savechance, 1, target, true);
		AbstractAttack.hit(a, chances, (hitchance - confirmchance) * nosavechance, 1, target, false);
		AbstractAttack.hit(a, chances, confirmchance * savechance, a.multiplier, target, true);
		AbstractAttack.hit(a, chances, confirmchance * nosavechance, a.multiplier, target, false);
		if (Javelin.DEBUG) {
	 ...
	 * @see #bind(float)
	 */
	public float misschance(final BattleState gameState, final Combatant current, final Combatant target,
			final int attackbonus) {
		return AbstractAttack.bind((target.ac() + getpenalty(current, target, gameState) - attackbonus) / 20f);
	}

	static void hit(final Attack a, final List<DamageChance> chances, final float hitchance, final int multiplier,
			Combatant target, boolean save) {
		if (hitchance == 0) {
			return;
		}
		for (final Entry<Integer, Float> roll : Action.distributeRoll(a.damage[0], a.damage[1]).entrySet()) {
			int damage = (roll.getKey() + a.damage[2]) * multiplier;
			if (damage < 1) {
				damage = 1;
			}
			chances.add(new DamageChance(hitchance * roll.getValue(), damage, multiplier != 1,
	...
	static int waterpenalty(final BattleState gameState, final Combatant current) {
		return current.source.swim() > 0 && gameState.map[current.location[0]][current.location[1]].flooded ? 2 : 0;
	}
	 ...
	 */
	public int getpenalty(final Combatant attacker, final Combatant target, final BattleState s) {
		return AbstractAttack.waterpenalty(s, attacker) - AbstractAttack.waterpenalty(s, target) + target.surprise()
				+ (target.burrowed ? 4 : 0);
	}

	ChanceNode createnode(final DamageChance dc, Combatant target, final BattleState gameState, Combatant attacker,
			final Attack attack, float ap) {
		String chance = " (" + getchance(attacker, target, attack, gameState) + " to hit)...";
		if (dc.damage == 0) {
			return new AttackNode(gameState, dc.chance, attacker + " misses " + target + chance, Delay.WAIT, target);
		}
		final BattleState attackstate = gameState.clone();
		attacker = attackstate.clone(attacker);
		target = attackstate.clone(target);
		StringBuilder messageAdd = new StringBuilder().append(attacker).append(" attacks ").append(target)
				.append(" with ").append(attack.name).append(chance).append("\n");
		if (dc.critical) {
		...
		if (dc.damage == 0) {
			...
			target.damage(dc.damage, attackstate, attack.energy ? target.source.energyresistance : target.source.dr);
		...
		return new AttackNode(attackstate, dc.chance, messageAdd.toString(), Delay.BLOCK, target);
	}

	void posthit(final DamageChance dc, Combatant target, Combatant attacker, final Attack attack, float ap,
			final BattleState attackstate, StringBuilder messageAdd) {
		if (target.hp > 0) {
			if (dc.save != null) {
				target.source = target.source.clone();
				attacker.source = attacker.source.clone();
				messageAdd.append("\n" + attack.effect.cast(attacker, target, attackstate, dc.save));
			}
		} else if (cleave()) {
			attacker.cleave(ap);
		}
	...
	public String getchance(Combatant current, Combatant target, Attack attack, BattleState s) {
		return Javelin.translatetochance(Math.round(20 * misschance(s, current, target, attack.bonus)));
	}
}