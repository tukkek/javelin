...
public abstract class Actor implements Serializable{
  ...
  protected void registerinstance(){
    ...
    var list=w.actors.get(getClass());
    if(list==null){
      list=new ArrayList<>(1);
      w.actors.put(getClass(),list);
    }
    if(!list.contains(this)) list.add(this);
    if(this instanceof Location) w.locations.put(new Point(x,y),(Location)this);
  }
  ...
  public void displace(int depth){
    ...
    actors.remove(this);
    if(World.get(p.x,p.y,actors)==null) move(p.x,p.y);
    else displace(depth+1);
  }

  protected boolean cancross(int tox,int toy){
    final var to=Terrain.WATER.equals(Terrain.get(tox,toy));
    if(!to) return true;
    final var from=Terrain.WATER.equals(Terrain.get(x,y));
    return from&&to;
  }
   ...
   *   <code>null</code> if this is not a valid request for this type of actor.
   *   For performance if a {@link List} is returned it should be the canonical
   *   one, and thus should not be directly modified by the receiver.
   * @see Combatant#newid()
   */
