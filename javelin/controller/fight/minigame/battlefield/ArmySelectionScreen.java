...
public class ArmySelectionScreen{
	...
	class BattleSize{
		...
		public String toString(){
			int from=dies;
			int to=dies*faces;
			return description+" (between "+from+" and "+to+" squads)";
		}
	}
	...
	public boolean selectarmy(BattlefieldFight f){
		...
		for(int el:els){
			...
			if(!pick(bluepickfirst,el,f)||!pick(!bluepickfirst,el,f)) return false;
		}
		return true;
	}
	...
	BattleSize choosesize(){
		...
		int choice=Javelin.choose("Choose the size of this battle:",sizes,true,
				false);
