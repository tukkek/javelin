...
public class Run extends Minigame{
	...
	class Segment{
		...
		public Point getpointa(){
			return new Point(x*SEGMENTSIZE,
					javelin.controller.map.Ziggurat.SIZE-(y+1)*SEGMENTSIZE);
		}
	...
	final HashMap<Integer,ArrayList<Monster>> pool=new HashMap<>();

	/**
	 * Organized with [0][0] being southwesternmost, which causes some funky math
	 * in comparison to normal map coordinates.
	 */
		...
		setup=new BattleSetup(){
			...
			public void place(){
				...
				for(Combatant c:state.blueTeam){
					...
					c.setlocation(getrandompoint(state,s.getpointa(),s.getpointb()));
				}
			}
	...
	void pool(){
		for(float cr:Javelin.MONSTERSBYCR.keySet()){
			List<Monster> tier=new ArrayList<>(Javelin.MONSTERSBYCR.get(cr));
			int round=Math.max(1,Math.round(cr));
			ArrayList<Monster> pooltier=pool.get(cr);
			if(pooltier==null){
				pooltier=new ArrayList<>(tier);
	...
	void createsegment(int x,int y,float cr){
		if(0<=x&&x<NSEGMENTS&&0<=y&&y<NSEGMENTS&&segments[x][y]==null){
	...
	public void endturn(){
		super.endturn();
		if(!state.redTeam.isEmpty()) return;
		if(disputed!=null){
			disputed.capture();
			recruit(getel());
			disputed=null;
	...
	void startbattle(Segment s){
		...
		if(!DEBUG){
			...
			for(Segment neighbor:new ArrayList<>(neighbors))
				if(neighbor.captured) neighbors.remove(neighbor);
			neighbors.add(s);
			while(state.redTeam.isEmpty()||ChallengeCalculator.calculateel(
					state.redTeam)<ChallengeCalculator.calculateel(state.blueTeam)-1){
				Segment neighbor=neighbors.isEmpty()?s:RPG.pick(neighbors);
	...
	Segment disputesegment(){
		for(Segment[] ss:segments)
			for(Segment s:ss){
				...
				Point b=s.getpointb();
				for(Combatant c:state.blueTeam)
					if(a.x<=c.location[0]&&c.location[0]<=b.x&&a.y<=c.location[1]
							&&c.location[1]<=b.y)
						return s;
			}
		return null;
	...
	void recruit(int targetel){
		...
		while(ChallengeCalculator.calculateel(state.blueTeam)<targetel){
			...
			Monster m=pool.get(Javelin.choose(
					"What unit do you want to recruit?\n\n"+currentteam,pool,true,true));
			Combatant c=new Combatant(m.clone(),true);
			c.setlocation(BattleSetup.getrandompoint(state,disputed.getpointa(),
					disputed.getpointb()));
			c.ap=state.next.ap;
			c.initialap=state.next.ap;
	...
	ArrayList<Monster> getrecruits(){
		...
		for(Segment[] ss:segments)
			for(Segment s:ss)
				if(s.captured) pool.add(s.monster);
		pool.sort((o1,o2)->o2.cr.compareTo(o1.cr));
		while(pool.size()>5)
			pool.remove(5);
		return pool;
	}
	...
	ArrayList<Segment> getneighbors(Segment s){
		ArrayList<Segment> neighbors=new ArrayList<>();
	...
	public void draw(){
		...
		for(Segment[] ss:segments)
			for(Segment s:ss)
				if(s.captured) s.discover();
	}

	@Override
	public boolean onend(){
		if(victory)
			Javelin.message("Congratulations, you have won!",true);
		else
