...
class ArmorClass extends FieldReader {
	...
	void read(final String value) {
		int extra = 0;
		for (final String acFactor : value.substring(value.indexOf("(") + 1,
				value.lastIndexOf(")")).split(",")) {

			final String trim = acFactor.trim();
			final String type = trim.substring(trim.indexOf(" ")).toLowerCase()
					.trim();
		...
		reader.monster.ac = Integer.parseInt(value.substring(0,
				value.indexOf(" ")));
	}
}