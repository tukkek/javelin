...
public class BattleState implements Node, TeamContainer {
    ...
    /**
     * Last LoS calculation. See {@link #haslineofsight(Point, Point, int, String)}.
     */
     ...
     * @see #clone()
     */
    public BattleState(final ArrayList<Combatant> blueTeam, final ArrayList<Combatant> redTeam,
	    ArrayList<Combatant> dead, ArrayList<Combatant> fleeing, final Square[][] map, String period,
	    ArrayList<Meld> meld) {
	this.map = map;
	this.period = period;
     ...
     * that anything that would cause an aoo is simply prohibited. since the game is
     * more fluid with movement/turns now this shouldn't be a problem.
     *
    ...
    /**
     * @param range            How many 5-feet squares ahead the active combatant
     *                         can see.
     * @param periodperception Represents the light perception for the active
     *                         combatant since some monsters may have the darkvision
     *                         quality or similar. If night or evening the character
     *                         will not able to see past obstacles in the map.
     */
    public Vision haslineofsight(final Point me, final Point target, int range, String periodperception) {
	...
	if (periodperception != Javelin.PERIODEVENING && periodperception != Javelin.PERIODNIGHT) {
	...
	return haslineofsight(new Point(me.location[0], me.location[1]), new Point(target.x, target.y), me.view(period),
		me.perceive(period));
    }
	...
	final ArrayList<Combatant> attackerteam = blueTeam.contains(attacker) ? blueTeam : redTeam;
	final ArrayList<Combatant> defenderteam = blueTeam.contains(target) ? blueTeam : redTeam;
	...
	return flank != null && !flank.burrowed && Walker.distance(target, flank) < 1.5 && attackerteam.contains(flank);
    }
     ...
     * (which would result in 2 different clones). For example: if you have a Caster
     * and a Target it just could be they are both the same unit (self-targetting)
     * and in this case you don't want to clone them twice.
     *
     ...
     *
     * @param team A list containing the combatants that should not be excluded from
     *             {@link #fleeing}.
