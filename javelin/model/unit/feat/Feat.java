...
public abstract class Feat implements Serializable,javelin.model.Cloneable{
	 ...
	 *
	 * Note that many feats do not have to implement this as the stat block itself
	 * will have the changes pre-computated in it. For example: {@link Toughness}
	 * and {@link ImprovedInitiative} only need to implement
	...
	public boolean upgrade(Combatant c){
		if(!stack&&c.source.hasfeat(this)) return false;
		if(prerequisite!=null&&!c.source.hasfeat(prerequisite)) return false;
	...
	/**
	 * @param monster Called during {@link MonsterReader} execution to give feats
	 *          a chance to make necessary adjustements to the monster being read.
	 */
