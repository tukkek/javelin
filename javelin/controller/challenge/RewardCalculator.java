...
public class RewardCalculator{
  ...
  static double getcharacterxp(int eldifference,final int nsurvivors){
    if(eldifference>4) eldifference=4;
    else if(eldifference<-12) eldifference=-12;
    return table.get(eldifference).getValue(nsurvivors);
  }

  static double getpartyxp(int eldifference,int nsurvivors,double bonus){
   ...
   * @see Combatant#xp
   */
  public static String rewardxp(List<Combatant> team,int el,double bonus){
    var elblue=ChallengeCalculator.calculateelfromcrs(
  ...
  public static void distributexp(List<Combatant> units,double xp){
    ...
    var leader=power.stream().max(Double::compare).orElse(null);
