...
/***Implements generation routines for Tyrant small towns*e.g.the villages in North Karrain**Towns are constructed out of a collection of 16*16 blocks*using semi-random layouts**

@author
Mike**/public class Town {
	...
	public static int[][][] towns = {
			{ { 11, 22, 16, -2, -1 }, { -2, 9, 6, 13, -2 },
					{ -2, 8, -2, 15, -1 } },
			{ { 8, -1, -2, -1, 7 }, { 9, 13, 6, -2, -1 },
					{ 17, 19, 12, -2, 14 } }
	...
	private static void buildTown(BattleMap m, int[][] areas) {
		...
		m.set("WallTile", RPG.pick(
				new int[] { Tile.CAVEWALL, Tile.WOODENWALL, Tile.STONEWALL }));
	 ...
	 * @see Person.java - Added a (blank) sign next to the door. - Removed the
	 *      small room on the side. - Added some common constants for code
	 *      maintaince.
	 */
	public static void buildTemple(BattleMap m, int x, int y) {
		...
		// Add the stone floor.
		m.fillArea(tmpX - left, tmpY1, tmpX + right, tmpY2, Tile.STONEFLOOR);
		// Make the stone wall border
		m.fillBorder(tmpX - left, tmpY1, tmpX + right, tmpY2, Tile.STONEWALL);
		...
		/*
		 * Add the healer The AI of the healer just like the shopkeeper. TODO:
		 * Might change to teacher script later.
		 */
		final int tmpY3 = y + 7;
		...
		/*
		 * Fires in the room TODO: Why fire won't display anymore?
		 */
	...
	private static void buildArea(BattleMap m, int x, int y, int t) {
		...
		case 0: {
			...
			switch (RPG.d(4)) {
			case 1:
				m.addThing(Lib.createType("IsWell", m.getLevel()), x, y, x + 15,
						y + 15);
				break;
			case 2:
				m.addThing(Lib.createType("IsGravestone", m.getLevel()), x, y,
						x + 15, y + 15);
				break;

			default:
				m.clearArea(x + 5, y + 5, x + 10, y + 9);
				m.fillArea(x + 5, y + 5, x + 10, y + 9, Tile.WOODENWALL);
				m.fillArea(x + 6, y + 6, x + 9, y + 8, Tile.WOODENFLOOR);
				m.setTile(x + 5, y + 7, Tile.WOODENFLOOR);
				m.addThing(Door.createDoor(5), x + 5, y + 7);
				m.addThing(Lib.createType("IsEquipment", 5), x + 5, y + 7);
			}
		...
		case 3: {
			...
			// block
		case 4: {
			addRoom(m, x + 8 - RPG.d(2, 3), y + 8 - RPG.d(2, 3), x + 8, y + 8,
					0, -1);
			addRoom(m, x + 8 - RPG.d(2, 3), y + 8, x + 8, y + 8 + RPG.d(2, 3),
					0, 1);
			addRoom(m, x + 8, y + 8 - RPG.d(2, 3), x + 8 + RPG.d(2, 3), y + 8,
					0, -1);
			addRoom(m, x + 8, y + 8, x + 8 + RPG.d(2, 3), y + 8 + RPG.d(2, 3),
					0, 1);
			break;
		}
		...
		case 6: {
			...
			addShop(m, x + 15 - RPG.d(3, 2), y, x + 15, y + RPG.d(3, 2), 0, 1,
					0);
			addShop(m, x, y + 15 - RPG.d(3, 2), x + RPG.d(3, 2), y + 15, 0, -1,
					2);
		...
		case 11: {
			...
			if ((t & 1) == 1) {
				// add the bridge
				m.replaceTiles(x, y + 7, x + 15, y + 7 + RPG.r(3), Tile.RIVER,
						Tile.STONEFLOOR);
			}
		...
		case 13: {
			...
			p1 = 7 - RPG.d(3);
			p2 = 7 + RPG.d(3);
			addFeature(m, x, y + RPG.r(2), x + p1, y + 5 + RPG.r(3), 0, 1, 0);
			addFeature(m, x + p1, y + RPG.r(2), x + p2, y + 5 + RPG.r(3), 0, 1,
					0);
			addFeature(m, x + p2, y + RPG.r(2), x + 14, y + 5 + RPG.r(3), 0, 1,
					0);

			p1 = 8 - RPG.d(4);
			p2 = 8 + RPG.d(4);
			addFeature(m, x, y + 9 + RPG.r(3), x + p1, y + 14 - RPG.r(2), 0, -1,
					0);
			addFeature(m, x + p1, y + 9 + RPG.r(3), x + p2, y + 14 - RPG.r(2),
					0, -1, 0);
			addFeature(m, x + p2, y + 9 + RPG.r(3), x + 14, y + 14 - RPG.r(2),
					0, -1, 0);
	...
	public static void addGraveYard(BattleMap m, int x1, int y1, int x2,
			int y2) {
		for (int i = 0; i < ((x2 - x1 + 1) * (y2 - y1 + 1)) / 8; i++) {
			m.addThing(Lib.createType("IsGraveStone", m.getLevel()), x1, y1, x2,
					y2);
		}
	...
	public static void addShop(BattleMap m, int x1, int y1, int x2, int y2, int
 dx,
 int dy, int t) {
 int doorx, doory;
 doorx = (dx == 0)
 ...
 * (RPG.r(2) * 2 - 1));
 ...
 Thing sk = Lib.create(RPG.pick(new String[] {"goblin shopkeeper","learned
 sage","shopkeeper","wizard"}));
 ...
 * (RPG.r(2) * 2 - 1));
 ...
 m.addThing(door, doorx, doory);
	...
	public static void addRoom(BattleMap m, int x1, int y1, int x2, int y2,
			int dx, int dy) {
		int doorx, doory;
		doorx = (dx == 0) ? RPG.rspread(x1 + 1, x2 - 1)
				: ((dx == -1) ? x1 : x2);
		doory = (dy == 0) ? RPG.rspread(y1 + 1, y2 - 1)
				: ((dy == -1) ? y1 : y2);
	...
	public static void addStandardRoom(BattleMap m, int x1, int y1, int x2,
			int y2, int doorx, int doory) {
		m.fillArea(x1, y1, x2, y2, Tile.FLOOR);
	...
	public static void decorateRoom(BattleMap m, int x1, int y1, int x2,
			int y2) {
		int w = x2 - x1 + 1;
		int h = y2 - y1 + 1;
		...
		case 2:
			if (w > 3 && h >= 3) {
				Point location =
						createPersonAndTable("townswoman", m, x1, y1, x2, y2);
				m.addThing(Lib.create("stool"), location.x + 1 - 2 * RPG.r(2),
						location.y);
				m.addThing(Lib.create("stool"), location.x,
						location.y + 1 - 2 * RPG.r(2));
		...
		case 3:
			if (w > 3 && h >= 3) {
				Point location = createPersonAndTable(
						RPG.pick(new String[] { "wizard", "priest" }), m, x1,
						y1, x2, y2);
				m.addThing(Lib.create("stool"), location.x + 1 - 2 * RPG.r(2),
						location.y + 1 - 2 * RPG.r(2));
				m.addThing(Lib.createMagicItem(3), x1, y1, x2, y2)
						.set("IsOwned", 1);
				m.addThing(Lib.createMagicItem(3), x1, y1, x2, y2)
						.set("IsOwned", 1);
				m.addThing(Scroll.createScroll(6), x1, y1, x2, y2)
						.set("IsOwned", 1);
				m.addThing(Scroll.createScroll(8), x1, y1, x2, y2)
						.set("IsOwned", 1);
			}
			break;
		case 4:
		...
		case 7:
			...
			m.addThing(Lib.createMagicItem(3), x1, y1, x2, y2).set("IsOwned",
					1);
			m.addThing(Scroll.createScroll(6), x1, y1, x2, y2).set("IsOwned",
					1);
			m.addThing(Scroll.createScroll(8), x1, y1, x2, y2).set("IsOwned",
					1);
			break;
		case 8:
			...
			break;
		case 9:
			m.addThing(Lib.createType("IsBook"), x1, y1, x2, y2).set("IsOwned",
					1);
			break;

		default:
			m.addThing(Lib.createItem(RPG.d(12)), x1, y1, x2, y2).set("IsOwned",
					1);
			break;
		}
	}

	private static Point createPersonAndTable(String toCreate, BattleMap m,
	...
	public static void decorateTown(BattleMap m, int x1, int y1, int x2,
			int y2) {
		for (int x = x1; x <= x2; x++) {
			for (int y = y1; y <= y2; y++) {
				if (m.isBlocked(x, y)) {
