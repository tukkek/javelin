...
public abstract class KeysScreen extends Frame{
  ...
  protected Container generate(){
    ...
    for(ActionDescription a:actions){
      ...
      final var key=new TextField(a.getMainKey(),1);
      content.add(key);
      keys.add(key);
    ...
    content.add(new Label());
    content.add(new Label());
    var restore=new java.awt.Button("Restore defaults (requires restart)");
    content.add(restore);
    restore.addActionListener(new RestoreDefault());
    var button=new java.awt.Button("Save");
    content.add(button);
  ...
  protected void enter(){
    ...
    for(var i=0;i<actions.size();i++){
      var textarea=keys.get(i);
      var key=textarea.getText();
