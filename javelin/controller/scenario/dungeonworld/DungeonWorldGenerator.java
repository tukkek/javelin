...
public class DungeonWorldGenerator extends WorldGenerator{
	...
	HashSet<Point> getfreespace(World w){
		HashSet<Point> free=new HashSet<>(World.scenario.size*World.scenario.size);
		...
		for(int x=0;x<World.scenario.size;x++)
			for(int y=0;y<World.scenario.size;y++){
				...
				}else if(t==Terrain.FOREST) free.add(new Point(x,y));
			}
		return free;
	}
	...
	void generatenewareas(World w){
		...
		List<Terrain> terrains=new ArrayList<>(Arrays.asList(Terrain.ALL));
	...
	void generatenewarea(Terrain t,HashSet<Point> region,int size,World w){
		...
		int limit=World.scenario.size-1;
		for(Point p:newarea)
			if(p.x!=0&&p.y!=0&&p.x!=limit&&p.y!=limit) w.map[p.x][p.y]=t;
	}

	/**
	 * Expands an area.
	 *
	 * @param pool Possibilites to start from.
	 * @param region Points that are not in this region will be discarded. Ignored
	 *          if <code>null</code>.
	 * @return A new point for the given area.
	 * @throws RestartWorldGeneration Through {@link WorldGenerator#retry()}.
	 */
