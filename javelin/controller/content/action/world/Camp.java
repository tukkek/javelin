...
public class Camp extends WorldAction{
  ...
  public void perform(WorldScreen screen){
    ...
    var t=(Town)Squad.active.findnearest(Town.class);
    if(t!=null&&t.getdistrict().getarea().contains(Squad.active.getlocation())){
      Javelin.message(INSIDETOWN,false);
      return;
    }
    String prompt;
    if(Javelin.DEBUG) prompt=PROMPTDEBUG;
    else prompt=WARNING+PROMPT;
    var period=PERIODS.get(Javelin.prompt(prompt));
    if(period==null) return;
    final var hours=period[0];
    final var rest=period[1];
