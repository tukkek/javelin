...
public class Burst extends Area {
	...
	public Burst(final int sourcex, final int sourcey, final int ax,
			final int ay, final int bx, final int by, final int cx,
			final int cy) {
	...
	@Override
	public Set<Point> fill(int range, final Combatant c,
	...
	private void recursivefill(final Point source, final Point current,
		...
		for (final Point direction : directions) {
			recursivefill(source,
					new Point(current.x + direction.x, current.y + direction.y),
					range, area, s);
		}
	}
}