...
public class Damage extends FieldReader {
	...
	public Damage(MonsterReader monsterReader, final String fieldname) {
		super(fieldname);
	...
	public void read(final String value) {
		// TODO implement
		for (final String blackList : new String[] { "poison", "transformation",
				"permanent", "fire", "corporeal instability", "acid", "cold",
				"mummy rot", "disease", "energy", "strand", "disruption",
				"slaying", "entangle", "fear", "stun", "paralysis",
				"implant" }) {
	...
	public void setdamage(final String attack, final Attack a) {
		try {
			a.damage = parsedamage(attack
					.substring(cutname(attack) + 1, attack.lastIndexOf(' '))
					.trim());
			a.getAverageDamageNoBonus();
			final String[] crit = attack
					.substring(attack.indexOf('(') + 1, attack.indexOf(')'))
					.split("/");
