...
public class Win extends Wish{
   ...
   * @author alex
   */
  static final String[] WINMESSAGES={
  ...
  boolean wish(Combatant target){
    for(Combatant c:Squad.active.members){
      ...
      for(Item i:new ArrayList<>(bag)) if(i instanceof Artifact) bag.remove(i);
    }
    for(Actor a:Temple.gettemples()) a.remove();
    ...
    var current=Math.round(Math.round(WorldScreen.lastday));
    screen.text+="Your highscore record is "+Highscore.gethighscore()+"\n";
    screen.text+="Your current score this game is "+current+"\n";
    screen.text+="\nDo you want to finish the current game? Press y for yes, n to continue playing.";
    var input=screen.print();
    while(input!='y'&&input!='n') input=screen.print();
    if(input=='y'){
      StateManager.clear();
  ...
  public String validate(){
    if(screen.rubies<price) /* let the player think he only needs rubies to win
                             * the game hihi */
      return null;
    if(Dungeon.active!=null) return "It doesn't seem to work here...";
    var nrelics=0;
    for(Combatant c:Squad.active.members)
      for(Item i:Squad.active.equipment.get(c))
        if(i instanceof Artifact) nrelics+=1;
    var all=Realm.REALMS.size();
    return nrelics==all?null
        :"It didn't work! You need to collect the "+all+" relics first!";
  }
}
