...
public class Movement extends Action{
	 ...
	 * @param descriptivekeys Used in printed description to user.
	 */
	 ...
	 */
	static public float cost(final Combatant c,final BattleState s,int x,int y,
			boolean disengaging){
		float speed;
		if(c.burrowed)
	...
	/**
	 * @param speed Speed in feet (5 feet = 1 square).
	...
	/**
	 * @param c Checks {@link Acrobatics}.
	...
	Point doDirection(final ActionDescription action,final BattleState state,
		...
		return Movement.tryMove(tox,toy,state,hero)?new Point(tox,toy):null;
	}
	 ...
	 * try to move
	 *
	 * @param state TODO use {@link Fight#state} directly.
	 * @param hero2
	 *
	...
	public static boolean tryMove(int x,int y,BattleState state,Combatant hero){
		...
		if(c.burrowed) Dig.refuse();
		javelin.controller.content.action.Movement.lastmovewasattack=true;
