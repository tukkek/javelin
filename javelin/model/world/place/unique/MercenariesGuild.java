...
/**
 * Allows a player to pick up fights, similar to an {@link Exhibition} but the
 * XP is doubled as an in-loco currency to generate an advanced mercenary when
 * requested.
 * 
...
public class MercenariesGuild extends UniqueLocation {
	...
	void generatemercenary() {
		...
		while (ChallengeRatingCalculator.calculateCr(c.source) < cr) {
			RPG.pick(UpgradeHandler.singleton.getupgrades(r)).upgrade(c);
			tries += 1;
			if (tries >= 100) {
				return;
			}
