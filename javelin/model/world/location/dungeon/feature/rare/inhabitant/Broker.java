...
public class Broker extends Inhabitant{
  ...
  class InhabitantScreen extends SelectScreen{
    List<Combatant> enemy=Arrays.asList(inhabitant);
    ...
    public List<Option> getoptions(){
      ...
      if(diplomacy>=diplomacydc) for(Key k:keys) options.add(new SellKey(k));
      if(hints>0) if(diplomacy>=diplomacydc) options.add(extractinformation);
      else if(diplomacy>=diplomacydc-5) options.add(sellinformation);
      options.add(attack);
      while(options.contains(null)) options.remove(null);
      return options;
    }
    ...
    boolean getinformation(){
      for(var i=0;i<hints;i++){
        var f=Dungeon.active.features.getundiscovered();
