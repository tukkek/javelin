...
public abstract class ShoppingScreen extends PurchaseScreen{
  ...
  public List<Option> getoptions(){
    ...
    for(final Item i:getitems()) list.add(new PurchaseOption(i));
    return list;
  }
  ...
  public String printpriceinfo(Option o){
    var i=((PurchaseOption)o).i;
    var useinfo="";
    if(i instanceof Wand||i instanceof Scroll){
      var members=new ArrayList<Combatant>(getbuyers());
      for(Combatant c:new ArrayList<>(members))
        if(i.canuse(c)!=null) members.remove(c);
      if(members.isEmpty()) useinfo=" - can't use";
      else{
        useinfo=" - can use: ";
        for(Combatant c:members) useinfo+=c+", ";
  ...
  protected Comparator<Option> sort(){
    return Comparator.comparing((Option a)->a.price).reversed();
  }
}
