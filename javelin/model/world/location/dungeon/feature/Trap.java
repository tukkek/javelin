...
public class Trap extends Feature {
     ...
     * new trap times should allow for lesser CRs.
     */
    ...
    int calculatecr() {
	return ratefactor(reflexdc) + ratefactor(searchdc) + ratefactor(disarmdc) + damagedie / 2;
    }
	...
	if (!success && searching != null) {
		...
		searchroll = Math.max(searchroll, searching.taketen(Skill.PERCEPTION) + bonus);
	    }
