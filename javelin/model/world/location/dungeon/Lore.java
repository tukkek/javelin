...
public class Lore implements Serializable{
  ...
  public boolean equals(Object lorep){
    if(!(lorep instanceof Lore lore)) return false;
  ...
  public static HashSet<Lore> generate(DungeonFloor floor){
    ...
    var monsters=floor.encounters.stream().filter(e->e!=null)
        .flatMap(Combatants::stream).map(c->c.source).filter(m->!m.elite)
    ...
    var features=floor.features.stream()
        ...
        .collect(Collectors.toList());
    for(var f:features) lore.add(new Lore(prefix+f,floor.dungeon));
    var items=floor.features.stream()
        ...
        .collect(Collectors.toList());
    for(var i:items) lore.add(new Lore(prefix+i.name,i.price));
    var artifacts=floor.features.stream().filter(f->f instanceof ArtifactChest)
        .collect(Collectors.toList());
    for(var a:artifacts) lore.add(new Lore(prefix+a,Integer.MAX_VALUE));
    return lore;
  }
}
