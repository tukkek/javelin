...
public class RuneGear extends Gear{
  ...
  public static void generate(){
    ...
    for(var s:Slot.SLOTS) if(s!=Slot.ARMS){//TODO
      var p=PRICES.get(s);
      for(var prefix:PREFIXES) new RuneGear(s,p).set(prefix);
      for(var suffix:SUFFIXES) new RuneGear(s,p).set(suffix);
    }
  }
  ...
  static int price(Condition prefix){
    ...
    if(prefix.longterm==null) p*=4;
    else if(prefix.longterm==0) p*=2;
    else if(prefix.longterm<=2) p*=1.5;
  ...
  public boolean equals(Object obj){
    return obj instanceof RuneGear g&&toString().equals(g.toString());
  }
