...
public abstract class Item implements Serializable,Cloneable,Healing{
  ...
  static{
    for(Tier t:Tier.TIERS) BYTIER.put(t,new ItemSelection());
  }
  ...
  public static void setup(){
    ...
    RuneGear.generate();
    Rune.generate();
    for(var i=0;i<ITEMS.size();i++) ITEMS.set(i,ITEMS.get(i).randomize());
    for(var i:ITEMS) BYTIER.get(Tier.get(i.getlevel())).add(i);
   ...
   *   {@link Double} makes it easier to just declare real numbers in such
   *   formulas, even though the final result will be passed to
   ...
   *   {@link Chest}, for example. Examples being: a used {@link Wand} with a
   *   certain amount of charges or a {@link Gem} with a random sell value.
   */
  ...
  public String toString(){
    return identified?name
        :"Unidentified "+getClass().getSimpleName().toLowerCase();
  }
  ...
  public void expend(){
    for(var c:Squad.active.members){
      ...
      for(var i:new ArrayList<>(bag)) if(i==this){
        bag.remove(i);
        return;
      }
    }
  ...
  public static List<Item> getplayeritems(){
    ...
    for(Actor a:World.getactors()){
      var academy=a instanceof Academy?(Academy)a:null;
      if(academy!=null){
        for(Order o:academy.training.queue){
          var training=(TrainingOrder)o;
          items.addAll(training.equipment);
        }
        continue;
      }
      var squad=a instanceof Squad?(Squad)a:null;
      if(squad!=null){
        squad.equipment.clean();
        for(List<Item> bag:squad.equipment.values()) items.addAll(bag);
        continue;
      }
    }
    return items;
  ...
  public static List<Item> randomize(Collection<Item> from){
    ...
    for(var i:from) randomized.add(i.randomize());
  ...
  public String describe(Combatant c){
    ...
    var prohibited=canuse(c);
    if(prohibited!=null) description+=" ("+prohibited.toLowerCase()+")";
  ...
  public boolean canuse(List<Combatant> members){
    for(var member:members) if(canuse(member)==null) return true;
    return false;
  }
