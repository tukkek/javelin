...
public class Slot implements Serializable{
	 ...
	 * battle gloves so to simplify we supposed the "magical hand energies" would
	...
	/**
	 * Only one finger on each hand is allowed at a time. Hand items also need to
	 * be removed.
	 */
	...
	public String toString(){
		if(equals(COLLAR)) return "Collar";
		if(equals(TORSO)) return "Torso";
		if(equals(ARM)) return "Arm";
		if(equals(WAIST)) return "Waist";
		if(equals(FEET)) return "Feet";
		if(equals(BACK)) return "Back";
		if(equals(HAND)) return "Hand";
		if(equals(FINGER)) return "Finger";
		if(equals(HEAD)) return "Head";
		if(equals(EYES)) return "Eyes";
