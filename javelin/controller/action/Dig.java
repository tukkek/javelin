...
public class Dig extends Action implements AiAction{
	...
	public boolean perform(Combatant hero){
		...
		if(flooded(hero,s)) return false;
	...
	public List<List<ChanceNode>> getoutcomes(Combatant active,BattleState s){
		ArrayList<List<ChanceNode>> outcomes=new ArrayList<>(1);
		if(active.source.burrow==0||flooded(active,s)) return outcomes;
		ArrayList<ChanceNode> outcome=new ArrayList<>(1);
		s=s.clone();
