...
public class Attacks extends FieldReader {
		...
		for (final String attackSequence : value.split(";")) {
			...
			for (final String attack : attackSequence.split(",")) {
				try {
					...
					reader.errorhandler
							.setInvalid("incorrect attack syntax: " + attack);
					return;
				}
			}
		}
	...
	void parseattack(final String attackp, final ArrayList<Attack> list) {
		String attack =
				cleanor(attackp.replace("ranged", "").replace("melee", ""));
		...
		if (touch) {
			modifiers = modifiers.replace("touch", "").trim();
		}
		attack += modifiers;
	...
	static public String cleanor(String s) {
		return s.replace(" or", "").replace("or ", "").replaceAll("  ", " ")
				.trim();
	}
	...
	private void checkprevious(final ArrayList<Attack> list,
		...
		for (final String previousName : previousNamesSet) {
			if (name.equalsIgnoreCase(previousName)
				...
				throw new RuntimeException("adding attack '" + name
						+ "': Monster '" + reader.monster
