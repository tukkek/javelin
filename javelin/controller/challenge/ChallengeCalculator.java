...
public class ChallengeCalculator{
  ...
  public static final List<Float> FRACTIONAL=List.of(3.5f,3f,2.5f,2f,1.75f,
      1.5f,1.25f,1f,.5f,0f,-.5f,-1f,-1.5f,-2f,-2.5f,-3f);
  ...
  public static final CrFactor[] CR_FACTORS={new SkillsFactor(),
  ...
  static final int[] XPPERLEVEL={0,1000,3000,6000,10000,15000,21000,28000,36000,
      45000,55000,66000,78000,91000,105000,12000,136000,153000,171000,190000,};
  static final int[] GOLDPERLEVEL={0,900,2700,5400,9000,13000,19000,27000,36000,
      49000,66000,88000,110000,150000,200000,260000,340000,440000,580000,
      760000,};
  ...
  static{
    ...
    var crs=new ArrayList<Float>(8+30);
    for(float fraction:new float[]{1/16f,1/12f,1/8f,1/6f,1/4f,1/3f,1/2f,2/3f})
      crs.add(fraction);
    for(var cr=1;cr<=30;cr++) crs.add(Float.valueOf(cr));
    for(float cr:crs){
      var el=crtoel(cr);
      var list=CRSBYEL.get(el);
      if(list==null){
  ...
  static public float calculatecr(final Monster m){
    ...
    var r=calculaterawcr(m);
    var goldenrule=r[1];
    var base=goldenrule>=4?Math.round(goldenrule):roundfraction(goldenrule);
    var cr=translatecr(base);
    m.cr=cr;
  ...
  public static float[] calculaterawcr(final Monster m){
    ...
    log(m.toString());
    final var factorHistory=new TreeMap<CrFactor,Float>();
    var cr=0F;
    for(final CrFactor f:CR_FACTORS){
      final var result=f.calculate(m);
  ...
  /**
   * @param cr Decimal CR.
   * @return Similar result from allowed {@link #FRACTIONAL}.
   */
  static float roundfraction(float cr){
    for(float limit:FRACTIONAL) if(cr>=limit) return limit;
  ...
  static private float goldenrule(final TreeMap<CrFactor,Float> factorHistory,
      final float cr){
    final var hpCheck=factorHistory.get(HIT_DICE_FACTOR)
        +factorHistory.get(CLASS_LEVEL_FACTOR);
    if(hpCheck<cr/2){
      final var twicehp=hpCheck*2;
  ...
  static float xptocr(int xp){
    var xptolevel=-1;
    var level=1;
    for(;level<20;level++) if(XPPERLEVEL[level]>=xp){
      xptolevel=XPPERLEVEL[level];
      break;
    }
    if(xptolevel==-1)
      throw new RuntimeException("Not enough levels #xptocr "+xp);
    return level*(xp/Float.valueOf(xptolevel))*(1-PCEQUIPMENTCRPERLEVEL);
  }
   ...
   *   prestige class this should be the minimum level required to enter that
   *   class.
   ...
   *   accessed. If from a prestige class, add the prestige class level to the
   ...
   *   starting levle it operates on. 0 otherwise. If the class is a prestige
   *   class this is the pure prestige class level (not added to minimumlevel).
   * @param casterlevel The current level this ability is operating on. 0 if not
   *   dependent upon level. If the class is a prestige class this is the pure
   *   prestige class level (not added to minimumlevel).
  ...
  public float rateability(int minimumlevel,int usesperday,int startinglevel,
    ...
    var access=minimumlevel*(startinglevel!=0?100:150);
    var basecost=access/2;
    var xp=access; // access cost
    for(var useperday=1;useperday<=usesperday;useperday++)
      xp+=useperday*basecost;
    for(var level=startinglevel+1;level<=casterlevel;level++)
      xp+=50*startinglevel;
   ...
   *   caster level.
   */
  public static float ratespell(int level,int casterlevel){
    if(level<1) level=1;
    if(casterlevel<1) casterlevel=1;
    return casterlevel*level*.01f;
  }
  ...
  public static void updatecr(ArrayList<Combatant> update){
    for(Combatant c:update) calculatecr(c.source);
  }
