 ...
 * {@link Transport} hub.
 *
 * TODO since other {@link Trait#EXPANSIVE} {@link Labor}s were removed,
 * probably make it start already at full capabilities. Should also drop
 * {@link Carriage} too?
 *
 * @author alex
 */
public class Hub extends Location{
  ...
  static final Option CARRIAGE=new Option("Carriage",Transport.CARRIAGE.price,
      'c');
  ...
  static final Option[] TRANSPORTATION={CARRIAGE,SHIP,AIRSHIP};
  static final String[] NAMES={"Stables","Docks","Magic dock"};
  ...
  class ShowTransport extends PurchaseScreen{
    ...
    public List<Option> getoptions(){
      ...
      if(Squad.active.transport==null)
        for(var i=0;i<=stables.level;i++) list.add(TRANSPORTATION[i]);
      return list;
    }
    ...
    public boolean select(Option o){
      ...
      if(o==REFUND) return refund(Squad.active);
      if(o==CARRIAGE) Squad.active.transport=Transport.CARRIAGE;
      else if(o==SHIP) Squad.active.transport=Transport.SHIP;
      else if(o==AIRSHIP) Squad.active.transport=Transport.AIRSHIP;
      else throw new RuntimeException("[TransportScreen] Unknown option");
    ...
    public boolean refund(Squad s){
      ...
      var p=Squad.active.getlocation();
  ...
  public static class BuildTransportHub extends Build{
    ...
    protected Point getsitelocation(){
      var sites=town.getdistrict().getfreespaces();
      for(Point p:sites) if(searchforwater(p)) return p;
      return sites.get(0);
    }
  }
  ...
  class DocksUpgrade extends BuildingUpgrade{
    ...
    public DocksUpgrade(int cost,int upgradelevel,Hub previous,
        Rank minimumsize){
      super("",cost-previous.cost,upgradelevel,previous,minimumsize);
    }
  ...
  public ArrayList<Labor> getupgrades(District d){
    ...
    if(searchforwater(getlocation())){
      if(level==0) upgrades.add(new DocksUpgrade(this));
      else if(level==1) upgrades.add(new MagicDockUpgrade(this));
    }else if(level==0) upgrades.add(new MagicDockUpgrade(this));
    return upgrades;
  }

  /** If not able already, allows this to rent {@link Airship}s. */
  public void upgradetomagicdocks(){
    if(level!=2) new MagicDockUpgrade(this).done();
  }
