...
public class Dungeon extends Location {
     ...
     *
     * Entries can be set to <code>null</code> when certain encounters are pacified.
     * If rolled, these will result in skipped encounters (ie: the {@link Squad} met
     * them but they weren't hostile). Pacified encounters do not carry over to the
     * next level.
     *
     * @see Leader
     */
    ...
    /**
     * Prevent generating the same type of item for different {@link Chest}s. Since
     * we're dealing with similar amounts of gold, this ends up happening to often
     * if left to randomness.
     *
    ...
    void knowfeatures() {
	int knowledge = Squad.active.getbest(Skill.KNOWLEDGE).taketen(Skill.KNOWLEDGE);
     ...
     * determination of how many {@link RandomDungeonEncounter}s should take for the
     * player to explore the whole level.
     *
     * Currently, the calculation is done by setting a goal of one fight per room on
     * average (so naturally, larger {@link DungeonTier}s will have more fights than
     * smaller ones). The formula takes into account
     * {@link DungeonScreen#VIEWRADIUS} instead of counting each step as a single
     * tile.
     *
     * Since a Squad of the dungeon's intended level cannot hope to clear a dungeon
     * if it's large (in average they can only take 4-5 encounters of the same EL),
     * this is then offset by placing enough fountains that would theoretically
     * allow them to do the one dungeon in one go.
     *
    ...
    void map() {
	DungeonTier tier = gettier();
	DungeonGenerator generator = DungeonGenerator.generate(tier.minrooms, tier.maxrooms,
		parent == null ? null : parent.tables);
    ...
    protected Combatants generateencounter(int level, List<Terrain> terrains) throws GaveUp {
	...
	for (int i = 0; i < ntraps; i++) {
	    int cr = level + Difficulty.get() + gettable(FeatureModifierTable.class).rollmodifier();
