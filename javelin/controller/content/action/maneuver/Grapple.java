...
public class Grapple extends ExpertiseAction{
  ...
  ChanceNode hit(Combatant current,Combatant target,BattleState s,float chance){
    ...
    var b=calculatesavebonus(target);
    var duration=Math.round(1/calculatesavechance(current,b));
    if(duration<1) duration=1;
    var message=current+" grapples "+target+" for "+duration+" turn(s)!";
  ...
  String constrict(Combatant current,Combatant target,int duration,
      BattleState s){
    var c=current.source.constrict;
    if(c==null) return "";
    target.damage(c.damage*duration,
        c.energy?target.source.energyresistance:target.source.dr,s);
