...
public class Breathe extends Action implements AiAction{
  ...
  public List<List<ChanceNode>> getoutcomes(final Combatant combatant,
    ...
    for(final BreathWeapon breath:combatant.source.breaths)
      for(final Area a:(breath.type==BreathArea.CONE?BURSTS:LINES).values()){
        var source=a.initiate(combatant);
  ...
  public boolean perform(Combatant hero){
    ...
    while(breath==null) breath=selectbreath(hero.source);
    clear();
    var a=selectarea(hero,breath);
    ...
    state=state.clone();
    var outcomes=breath(breath,a,state.clone(hero),state);
  ...
  Area selectarea(Combatant hero,BreathWeapon breath){
    Javelin.message("Select a direction or press q to quit.",
        Javelin.Delay.NONE);
    Area a=null;
    while(a==null) a=(breath.type==BreathWeapon.BreathArea.CONE?BURSTS:LINES)
  ...
  String targetinfo(BattleState state,Set<Point> area){
    var targets="";
    for(Point p:area){
      var c=state.getcombatant(p.x,p.y);
      if(c!=null) targets+=c+", ";
    }
    if(!targets.isEmpty())
  ...
  static ArrayList<ChanceNode> breath(final BreathWeapon breath,final Area a,
    ...
    final var chances=new ArrayList<ChanceNode>();
    final var targets=new ArrayList<Combatant>();
    final var area=a.fill(breath.range,active,s);
    for(final Point p:area){
      final var target=s.getcombatant(p.x,p.y);
  ...
  static void hit(Combatant target,final int damage,final BattleState s,
    ...
    var savedc=breath.save(target);
    final float savechance;
    if(savedc==null) savechance=0;
    else{
    ...
    }else damage(target,damage,s,affected);
  }
  ...
  BreathWeapon selectbreath(Monster m){
    ...
    var index=Javelin.choose("Select a breath attack or press q to quit",
  ...
  public Integer selectdirection(Combatant hero){
    var direction=Javelin.input();
    var code=direction.getKeyCode();
    var key=direction.getKeyChar();
    quit(key);
