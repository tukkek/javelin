...
class Attacks extends FieldReader {
	private final MonsterReader monsterReader;

	Attacks(MonsterReader monsterReader, final String fieldname) {
		super(fieldname);
	...
	private void parseattack(final String attackp, final ArrayList<Attack> list) {
		...
		for (final int bonus : parsebonuses(attack.substring(attack
				.lastIndexOf(" ") + 1))) {
	...
	static Integer[] parsebonuses(final String bonusesP) {
		...
		final Integer[] array = new ArrayList<Integer>()
				.toArray(new Integer[split.length]);
	...
	private void checkprevious(final ArrayList<Attack> list, final String name) {
		...
		for (final String previousName : previousNamesSet) {
			if (name.equalsIgnoreCase(previousName)
				...
				MonsterReader
						.log("WARNING adding attack '"
								+ name
								+ "': Monster '"
								+ monsterReader.monster
								+ "' already has similar-named attack ('"
								+ previousName
	 ...
	 */
	public AttackSequence check(final String value, final String attackSequence) {
