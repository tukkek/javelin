...
public class LaborDeck extends ArrayList<Labor>{
  ...
  static final Labor[] MAGICAL={new BuildMagesGuild(),new BuildArtificer(),
      new BuildSummoningCircle(),new BuildBardsGuild(),new BuildRuneShop()};
  ...
  static{
    ...
    for(String title:new ArrayList<>(DECKS.keySet())){
      ...
      var t=new Trait(title,DECKS.get(title));
      TRAITS.add(t);
      DEFAULT.add(t);
    }
  }
   ...
   *   decks shuffled into tt. If the town has no {@link Trait}s, will only have
   *   the {@link #DEFAULT} {@link Labor} cards.
   */
  public static ArrayList<Labor> generate(Town t){
    var d=new LaborDeck();
    d.addAll(DEFAULT);
    for(String trait:t.traits) d.addAll(DECKS.get(trait));
  ...
  public static boolean isbasic(Labor card){
    for(Labor l:DEFAULT) if(card.getClass().equals(l.getClass())) return true;
    return false;
  }

  /** @see ContentSummary */
  public static String getsummary(){
    var count=DEFAULT.size();
    for(var d:DECKS.values()) count+=d.size();
