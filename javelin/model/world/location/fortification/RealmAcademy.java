...
public class RealmAcademy extends Academy{
	public static class BuildRealmAcademy extends BuildAcademy{
		...
		protected RealmAcademy generateacademy(){
			return new RealmAcademy(town.originalrealm,false);
		}
	 ...
	 *
	 * @param r Type of upgrade to offer. If you choose to set this as
	 ...
	 * @see UpgradeHandler
	 */
	public RealmAcademy(Realm r,boolean first){
		...
		maxlevel+=1;
		refill();
		if(first) upgrades.add(Skill.SURVIVAL.getupgrade());
	}

	protected void refill(){
		ArrayList<Upgrade> upgrades=new ArrayList<>(getupgrades(upgradetype));
		if(this.upgrades.isEmpty()) for(Upgrade u:upgrades)
		...
		for(Upgrade u:upgrades){
			if(this.upgrades.size()>=level) break;
			this.upgrades.add(u);
		}
	}
