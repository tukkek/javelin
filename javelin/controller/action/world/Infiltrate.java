...
public class Infiltrate extends WorldAction{
	...
	/** Constructor. */
	public Infiltrate(){
		super("Infiltrate guarded location'",new int[]{'I'},new String[]{"I"});
	}

	@Override
	public void perform(WorldScreen screen){
		Location target=selecttarget();
		if(target!=null) infiltrate(selectspy(target),target);
	}
	...
	Location selecttarget(){
		...
		for(int x=Squad.active.x-1;x<=Squad.active.x+1;x++)
			for(int y=Squad.active.y-1;y<=Squad.active.y+1;y++){
				...
				if(l!=null&&l.ishostile()) targets.add(l);
			}
		if(targets.isEmpty()){
		...
		int choice=Javelin.choose("Which location to infiltrate?",targets,true,
				false);
	...
	void infiltrate(Combatant spy,Location l){
		...
		while(true){
			...
			ArrayList<String> description=new ArrayList<>(targets.size());
			for(Combatant c:targets)
				description.add(c+" (difficulty: "
						+Javelin.describedifficulty(calculatestealthdc(spy,c))+")");
			int choice=Javelin.choose((killed?"Success!":"Assassinate who?")+immune,
			...
			killed=true;
			if(l.garrison.isEmpty()) break;
		}
	}

	void die(Combatant spy){
		Javelin.show(
				spy+" was discovered and executed!\n\nPress ENTER to continue...");
	...
	Combatant selectspy(Location target){
		ArrayList<Combatant> spies=new ArrayList<>(Squad.active.members);
		spies.sort((a,b)->Skill.DISGUISE.getbonus(a)-Skill.DISGUISE.getbonus(b));
		ArrayList<String> choices=new ArrayList<>(spies.size());
		for(Combatant c:spies){
			Monster m=c.source;
			choices.add(c+" (difficulty: "
					+Javelin.describedifficulty(calculatedisguiseroll(c,target))+")");
		}
		int choice=Javelin.choose(
				"Who will infiltrate the target: "+target.toString()+"?",choices,true,
				false);
	...
	public boolean assassinate(Combatant spy,Combatant target,Location l){
		...
		if(!roll(dc)) return false;
		l.garrison.remove(target);
		if(l.garrison.isEmpty()) l.capture();
		return true;
	}
