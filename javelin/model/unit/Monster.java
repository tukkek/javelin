...
public class Monster implements Cloneable,Serializable{
	...
	public enum MonsterType{
		ABERRATION,ANIMAL,CONSTRUCT,DRAGON,ELEMENTAL,FEY,GIANT,HUMANOID,MAGICAL_BEAST,MONSTROUS_HUMANOID,OOZE,OUTSIDER,PLANT,SHAPECHANGER,UNDEAD,VERMIN
	}
	...
	static private ArrayList<AttackSequence> copyattacks(
		...
		final ArrayList<AttackSequence> copy=new ArrayList<>(original.size());
	...
	public int countfeat(final Feat f){
		...
		for(final Feat existing:feats)
			if(existing.name.equals(f.name)) i+=1;
		return i;
	}

	public boolean hasfeat(final Feat f){
		for(final Feat existing:feats)
			if(existing.equals(f)) return true;
		return false;
	}
	 ...
	 *          bonus. A value of {@link Integer#MAX_VALUE} represents automatic
	 *          success.
	...
	public void changestrengthscore(int scorechange){
		AbilityModification m=AbilityModification.modify(strength,scorechange);
		strength=m.newscore;
		...
		for(AttackSequence sequence:melee)
			attacks:for(Attack a:sequence){
				for(WeaponFinesse wf:finesses)
					if(wf.affects(a)) continue attacks;
	...
	public void changedexterityscore(int scorechange){
		AbilityModification m=AbilityModification.modify(dexterity,scorechange);
	...
	public void changeconstitutionscore(Combatant c,int scorechange){
		AbilityModification m=AbilityModification.modify(constitution,scorechange);
	 ...
	 *         (which is not the case for animals that can only follow a few
	 *         specific, well-trained tasks).
	 */
	...
	/**
	 * @return <code>true</code> if the monster can swim on (or fly above) water.
	 */
	public int swim(){
