...
public class GoblinVillage {
	public static BattleMap makeGoblinVillage() {
		...
		m.makeRandomPath(w / 2, 0, w / 2, h / 2, 0, 0, w - 1, h - 1, m.floor(),
				false);
		...
		for (int i = 1; i < ps.length; i++) {
			int j = RPG.r(i);
			m.makeRandomPath(ps[i].x, ps[i].y, ps[j].x, ps[j].y, 0, 0, w - 1,
					h - 1, m.floor(), false);
		}
	...
	public static void makeChiefHut(BattleMap m, int x, int y) {
		...
		for (int i = RPG.d(2, 4); i > 0; i--) {
			Thing g = Lib.create(RPG.pick(new String[] { "goblin chieftain",
					"goblin shaman", "goblin hero" }));
