...
public class WorldMouse extends Mouse {
	...
	public void run() {
		...
		if (!WorldMove.move(p.x, p.y)) {
		    ...
		    p = overlay.path.resetlocation();
		    if (p != null) {
			overlay.reset();
			overlay.path.from.x = p.x;
			overlay.path.from.y = p.y;
			overlay.walk();
			MapPanel.overlay = overlay;
		    }
		    return;
		}
//		OverlayStep step = (OverlayStep) overlay.steps.get(i);
//		if (!step.safe && World.scenario.worldencounters) {
//		    RandomEncounter.encounter(step.hours / WorldScreen.HOURSPERENCOUNTER);
//		}
	    }
	    BattleScreen.active.mappanel.refresh();
	}
	...
	if (overlay == null || overlay.steps.isEmpty()) {
	    return false;
	}
