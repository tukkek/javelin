...
public abstract class Haunt extends Fortification{
  ...
  class HauntScreen extends SelectScreen{
    ...
    public List<Option> getoptions(){
      var hirable=new HashSet<>(hires).stream().sequential()
          .map(RecruitOption::new).collect(Collectors.toList());
    ...
    public String printpriceinfo(Option o){
      ...
      else price="$"+Javelin.format(o.price)+"/day";
      return " ("+price+")";
    }
    ...
    public boolean select(Option o){
      ...
      if(!MercenariesGuild.recruit(new Combatant(h,true),true,false)){
  ...
  protected Haunt(String description,Class<? extends LocationMap> map,
    ...
    while(RPG.chancein(2)&&tieri<Tier.TIERS.size()-1) tieri+=1;
  ...
  public int getminimumel(){
    ...
    for(var m:pool) if(m.cr<min) min=m.cr;
  ...
  void add(Set<Monster> defeated){
    ...
    for(var h:defeated){
      ...
      for(var i=0;i<quantity;i++) hires.add(h);
    }
  ...
  protected static boolean include(Monster m,List<String> subtypes){
    ...
    for(String subtype:m.subtypes) if(subtypes.contains(subtype)) return true;
    return false;
  }
  ...
  public static int getjoinfee(Monster m){
    var cr=Math.round(m.cr);
    if(cr<1) cr=1;
    return Tier.get(cr).getordinal()+1;
  }
}
