...
public abstract class Strike extends Maneuver{
  ...
  public boolean perform(Combatant c){
    /**
     * TODO implementing reach may change this? how does engagement work with
     * reach?
     */
    var engaged=Fight.state.isengaged(c);
    var attacks=engaged?c.source.melee:c.source.ranged;
    var choices=attacks.stream().map(a->a.get(0).toString()).toList();
    var i=Javelin.choose("Select attack:",choices,false,false);
    var sequence=attacks.get(i);
  ...
  public List<List<ChanceNode>> getoutcomes(Combatant c,BattleState s){
    var outcomes=new ArrayList<>(new MeleeAttack(this).getoutcomes(c,s));
