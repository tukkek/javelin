 ...
 * @author alex
 */
public class Dwelling extends GuardedPlace {
	/** Raise spawning chance and start without a garrison. */
	...
	public Dwelling() {
		super(null, null, 0, 0);
		descriptionknown =
				"A " + dweller.toString().toLowerCase() + " dwelling";
	...
	protected void generategarrison(int minel, int maxel) {
		...
		monsters: for (Monster m : Javelin.ALLMONSTERS) {
			for (String terrain : Javelin.terrains(Javelin.terrain(x, y))) {
