...
public class WorldScreen extends BattleScreen{
  ...
  void act(){
    try{
      ...
      var s=Squad.active;
      for(var squad:Squad.getsquads()) if(squad!=s&&s.gettime()!=squad.gettime()
          &&squad.getlocation().equals(s.getlocation()))
        s.join(squad);
      var d=s.getdistrict();
  ...
  void dofights(long time,ArrayList<Incursion> incursions){
    for(var i:RPG.shuffle(incursions)) i.turn(time,this);
    for(var t:RPG.shuffle(Town.gettowns())) t.dealevent();
  }
  ...
  public void updateplayerinformation(){
    ...
    if(s==null) return;
    MessagePanel.active.clear();
    final var infos=new ArrayList<String>();
    var date="Day "+currentday();
    if(Dungeon.active==null){
      infos.add(date+", "+Period.now().toString().toLowerCase());
      var season=Season.current.toString();
      var weather=Terrain.current().describeweather();
      if(!weather.isEmpty()) season+=", "+weather+"";
      infos.add(season);
    }else infos.add(Dungeon.active.toString());
    infos.add("");
    if(Dungeon.active==null){
      final var mph=s.speed(Terrain.current(),Squad.active.x,Squad.active.y);
  ...
  static String printgold(){
    final var upkeep=Squad.active.getupkeep();
  ...
  static private boolean checkexhaustion(Combatant m){
    for(Spell s:m.spells) if(!s.exhausted()) return false;
    return true;
  }
  ...
  public boolean explore(int x,int y){
    ...
    var hazards=Terrain.get(x,y).gethazards(special).stream()
        .filter(Hazard::validate).collect(Collectors.toList());
