...
public class Rune extends Item{
  ...
  public static void generate(){
    for(var p:RuneGear.PREFIXES) new Rune(p,null);
    for(var s:RuneGear.SUFFIXES) new Rune(null,s);
  }
  ...
  public boolean usepeacefully(Combatant user){
    ...
    var g=gear.get(choice);
    var from=g.toString();
    if(prefix!=null) g.set(prefix);
    else g.set(suffix);
    Javelin.message("%s is now a %s!".formatted(from,g),true);
    return true;
  }
}
