 ...
 * be easy to continually step on traps to plunder xp. TODO maybe when disarm
 * device and search skills are implemented we could move to single-use traps,
 * which would maybe also allow for more variety
 * 
 * TODO search + disarm (assuming balance will be even)
 * 
...
public class Trap extends Feature {
	...
	public boolean activate() {
		...
		if (found && disarm >= 0) {
			InfoScreen.prompt("You disarm the trap!");
			super.remove();
			return true;
		}
		if (!found || disarm <= -5 || InfoScreen
