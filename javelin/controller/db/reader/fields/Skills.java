...
public class Skills extends FieldReader{
	...
	public static final HashSet<String> UNKNOWN=Javelin.DEBUG?new HashSet<>()
			:null;
	...
	public void read(String text)
		...
		for(String skill:text.split(",")){
			...
			int value=Integer.parseInt(split[split.length-1].replace("*",""));
			apply(m,skill,value);
		}
	}
	...
	static void apply(Monster m,String skill,int value){
		...
		while(skill.contains("(")){
			String detail=skill.substring(skill.indexOf("("),skill.indexOf(")")+1);
		...
		else if(Javelin.DEBUG) UNKNOWN.add(skill);
	}
}
