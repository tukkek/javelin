...
public class Leader extends Inhabitant {
	...
	class Treaty extends Option {
		...
		Treaty(int price, String monster, Boolean lawful, Boolean good) {
			...
			if (monster != null) {
				...
				name += (good ? "good" : "evil") + " inhabitants";
			}
		}
		...
		boolean applies(Combatants encounter) {
			...
			for (Combatant c : encounter) {
				if (monster != null) {
					if (monster.equals(c.source.name)) {
						return true;
					}
				} else if (lawful != null) {
					if (lawful.equals(c.source.lawful)) {
						return true;
					}
				} else if (lawful != null && good.equals(c.source.good)) {
					return true;
				}
			}
			return false;
		...
		void register(ArrayList<Combatants> encounters,
				ArrayList<Option> options) {
			if ((lawful == null || lawful.equals(base.lawful))
					&& (good == null || good.equals(base.good))
					&& applies(encounters)) {
				options.add(this);
			}
		}
	...
	class LeaderScreen extends SelectScreen {
		...
		public String printinfo() {
			...
			if (Javelin.DEBUG) {
				...
				debug += "Your diplomacy: " + Squad.active
						.getbest(Skill.DIPLOMACY).taketen(Skill.DIPLOMACY)
						+ "\n";
				debug += "Alignment: " + base.lawful + " " + base.good;
				debug += "\n\n";
			}
			return debug + "Your squad has $"
		...
		List<Option> addtreaties(ArrayList<Option> options, int diplomacy) {
			...
			int treatyprice = 10 * Javelin
					.round(RewardCalculator.getgold(inhabitant.source.cr));
			new Treaty(treatyprice, base.name, null, null).register(encounters,
					options);
			if (diplomacy < diplomacydc + 5) {
				return options;
			}
			treatyprice *= 5 + Dungeon.active.gettier().tier;
			new Treaty(treatyprice, null, base.lawful, null)
					.register(encounters, options);
			new Treaty(treatyprice, null, null, base.good).register(encounters,
					options);
			return options;
		}
	...
	/**
	 * First entry is the Leader himself. Others are #base clones added to
	 * ensure a high-level encounter.
	 */
