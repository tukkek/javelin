...
public abstract class UpgradingScreen extends SelectScreen{
  public class UpgradeOption extends Option{
    ...
    /** Constructor. */
    public UpgradeOption(Upgrade u){
      super(u.name,0);
      this.u=u;
      var s=u instanceof Spell?(Spell)u:null;
      if(s!=null){
  ...
  HashMap<Integer,Combatant> original=new HashMap<>();
  HashSet<Combatant> upgraded=new HashSet<>();
  ...
  public void show(){
    if(original.isEmpty())
      for(Combatant c:gettrainees()) original.put(c.id,c.clone().clonesource());
    super.show();
  }
  ...
  public boolean select(Option op){
    var o=(UpgradeOption)op;
    List<Combatant> eligible=new ArrayList<>();
    var listeligible=listeligible(o,eligible);
    if(eligible.isEmpty()){
      print(text+"\nNone can learn this right now...\n");
      return false;
    }
    Combatant c;
    if(eligible.size()==1&&skipselection) c=eligible.get(0);
    else{
  ...
  Combatant selecttrainee(List<Combatant> eligible,String prompt){
  ...
  void finishpurchase(UpgradeOption o,Combatant c){
  ...
  void update(Combatant c){
    for(Feat f:c.source.feats) f.update(c);
  }

  String listeligible(UpgradeOption o,List<Combatant> eligible){
    var s="\n";
    var i=1;
    for(Combatant c:gettrainees()){
      var name=c.toString();
      while(name.length()<=10) name+=" ";
      var cost=buy(o,c.clone().clonesource(),true);
      if(cost!=null&&cost.compareTo(new BigDecimal(0))>0
        ...
        i++;
        var days=getperiod(cost.floatValue());
  ...
  BigDecimal buy(UpgradeOption o,Combatant c,boolean listing){
    ...
    if(!listing){
      var goldpieces=price(cost.floatValue());
  ...
  public List<Option> getoptions(){
    ...
    var options=new ArrayList<Option>(upgrades.size());
    for(Upgrade u:upgrades) if(u.showupgrade()) options.add(createoption(u));
    return options;
  }
  ...
  @Override
  public void onexit(){
    var trainees=new ArrayList<TrainingOrder>();
    for(var c:upgraded){
      var original=this.original.get(c.id);
      var xpcost=ChallengeCalculator.calculaterawcr(c.source)[1]
          -ChallengeCalculator.calculaterawcr(original.source)[1];
      trainees.add(createorder(c,original,xpcost));
    }
    onexit(trainees);
    for(TrainingOrder trainee:trainees) registertrainee(trainee);
  }
