...
public class Maze extends Caves{
	...
	public void generate(){
		...
		int finishedCount=0;
		for(int i=1;i<center*center*1000&&finishedCount<center*center;i++){
			int x=0+2*RPG.r(center);
			int y=0+2*RPG.r(center);
			if(!map[x][y].blocked) continue;
			int dx=RPG.r(1,2)==1?RPG.r(2)*2-1:0;
			int dy=dx==0?RPG.r(2)*2-1:0;
			int lx=x+dx*2;
			int ly=y+dy*2;
			if(lx>=0&&lx<=SIZE-1&&ly>=0&&ly<=SIZE-1) if(!map[lx][ly].blocked){
