...
public class BattleMouse extends Mouse {
    static final BattleMouseAction[] ACTIONS = new BattleMouseAction[] { new MoveMouseAction(), new MeleeMouseAction(),
	...
	Runnable outcome = action == null ? null : action.act(current, target, s);
	if (outcome != null) {
    ...
    public BattleMouseAction getaction(final BattleState s, final Combatant target, final Combatant current) {
	BattleMouseAction custom = target == null ? null : target.getmouseaction();
	...
	BattleScreen.active.mappanel.center(BattlePanel.current.location[0], BattlePanel.current.location[1], true);
    }
}