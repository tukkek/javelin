...
public class Caves extends Map {
	...
	/**
	 * Makes all map {@link Square} blocked and creates an inner room according
	 * to {@link #coresize}.
	 */
	protected void init() {
	...
	public Point findEdgeSquare(final int dx, final int dy) {
		for (int i = 0; i < 10 * SIZE * SIZE; i++) {
			...
			if (!map[x][y].blocked && map[x + dx][y + dy].blocked) {
	...
	void makeCrevice(int x, int y, int dx, int dy) {
		if (isBlank(x + dy, y - dx, x - dy, y + dx)) {
			return;
		}
