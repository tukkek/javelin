...
public class Skill implements Serializable {
	 ...
	 */
	public static final HashMap<Skill, SkillUpgrade> UPGRADES = new HashMap<Skill, SkillUpgrade>();
	/**
	...
	public class SkillUpgrade extends Upgrade {
		...
		public String inform(Combatant c) {
			return "Currently: " + Skill.this.name + " " + getsignedbonus(c)
					+ " (untrained)";
		}
		...
		protected boolean apply(Combatant c) {
			...
			if (m.trained.size() >= 3 + Monster.getbonus(m.intelligence)) {
				return false;
			}
			if (!canuse(c)) {
				return false;
