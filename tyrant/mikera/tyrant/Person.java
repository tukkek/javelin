 ...
 * @author Mike
 * 
 * This class contains frienly "Characters" used to populate towns , etc.
 */
public class Person {
    ...
    public boolean handle(Thing t, Event e) {
      ...
      if (h.getStat(sk) > 0) {
        if (get("Practitioner") != null) {
          Game.messageTyrant("\"Good day to you, fellow " + get("Practitioner") + ".\"");
        } else {
        ...
        if (h.getStat(RPG.ST_SKILLPOINTS) >= 2) {
          if (Coin.getMoney(h) < price) {
            Game.messageTyrant(t.getTheName() + " could teach you the " + sk + " skill for " + Coin.valueString(price) + "");
            Game.messageTyrant("Unfortunately, you do not have enough money to pay for this");
          } else {
            // can learn skill
            Game.messageTyrant(t.getTheName() + " is willing to help you acquire new knowledge.");
            Game.messageTyrant("Would you like to learn the " + sk + " skill for " + Coin.valueString(price) + "? (y/n)");
            char c = Game.getOption("yn");
            if (c == 'y') {
              h.incStat(sk, 1);
              h.incStat(RPG.ST_SKILLPOINTS, -2);
              Coin.removeMoney(h, price);
              Game.messageTyrant("You learn the " + sk + " skill from " + t.getTheName());
            } else {
          ...
          Game.messageTyrant(t.getTheName() + " can teach the " + sk + " skill for " + Coin.valueString(price) + "");
  ...
  private static class ListSellingScript extends Script {
    ...
    public boolean handle(Thing t, Event e) {
      ...
      for (Thing gift = Game.selectSaleItem("Select an item to sell:", h, t); //Declaration
                                                                              // part
                                                                              // of
                                                                              // for
                                                                              // loop
      ...
      gift = Game.selectSaleItem("Select an item to sell:", h, t)) { //Updater
        ...
        Game.messageTyrant("You sell your " + gift.getName(Game.hero(), count) + " to " + t.getTheName());
        if (count > 0){
          gift.remove(count);
        }
    ...
    public boolean handle(Thing t, Event e) {
      ...
      if (gift != null) {
        ...
        if (value > 0) {
          ...
          if (k == 'y') {
            Game.messageTyrant("You sell your " + gift.getName(Game.hero()) + " to " + t.getTheName());
            gift.remove();
  ...
  private static class RepairScript extends Script {
    ...
    /*
     * @author ppirrip 13, Aug, 04 TODO Make the smith repair damaged items with
     * a price. Based on the swap script should be sufficient. NOTE Might not
    ...
    public boolean handle(Thing t, Event e) {
      ...
      /*
       * // Debugging System.out.println("Current HPS = "+ gift.getStat("HPS"));
       * System.out.println("Max HPS = "+ gift.getStat("HPSMAX")); if
       * (damagedFlag) { System.out.println("True [ppirrip]"); } else {
      ...
      if (gift != null && damagedFlag) {
        ...
        if (value > 0) {
          giver.message(t.getTheName() + " asks for " + Coin.valueString(value)
              + " to repair your " + gift.getName(Game.hero()));
          giver.message("Do you accept? (y/n)");
          char k = Game.getOption("ynq");
          if (k == 'y') {
            Game.messageTyrant("You gave " + gift.getName(Game.hero()) + " and "
                + Coin.valueString(value) + " to the smith for repair.");
  ...
  private static class IdScript extends Script {
    ...
    public boolean handle(Thing t, Event e) {
      ...
      char k = Game.getOption("ynq");
      if (k == 'y') {
        Game.messageTyrant("You gave " + Coin.valueString(value) + " to the "
            + t.toString() + " to identify the " + gift.getName(Game.hero())
            + ".");
        ...
        // Identify the item
        Item.identify(gift);
        Game.messageTyrant(t.getTheName() + " identifies it as " + gift.getAName() + ".");
      } else {
        // you don't want to pay?
        giver.message("Don't waste my time then!");
      }
  ...
  private static class TeachingScript extends Script {
    ...
    public boolean handle(Thing t, Event e) {
      ...
      if (cost > 0) {
        Game.messageTyrant(t.getTheName() + " asks for " + Coin.valueString(cost) + " for training.");
      ...
      ArrayList sks = new ArrayList(Arrays.asList(Skill.getTrainableSkills(
          giver, true))); //Skill.getList(giver);
      ArrayList sksr = new ArrayList(Arrays.asList(Skill.getTrainableSkills(
          giver, false))); //Skill.getUnmarkedList(giver);
      ...
      if (s != null) {
        ...
        final String laterThen[] = { "Maybe later then!",
            "Well, maybe later then.", "Anyway, you can come back later." };
  ...
  private static class RiddleScript extends Script {
    ...
    public boolean handle(Thing t, Event e) {
      ...
      if (failures > 2) {
        ...
        Game.messageTyrant(t.getTheName() + " is no longer willing to talk with you.");
        return true;
      } else if (failures > 0) {
        Game.messageTyrant(t.getTheName() + " has another question for you.");
      } else {
  ...
  public static void initTownies() {
    ...
    t.set("Level", 10);
    t.set("Introduction", "Hi, I'm the town's ranger. I have arrows for sale.");
    ...
    t.set(Skill.CASTING, 3);
    t.set("ARM", 4);
    t.set("Introduction", "Hi, I'm the town's wizard. I have wands for sale.");
