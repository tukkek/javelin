...
public class Tile {
	...
	// public static final int TF_LOS = ;
	public static final int TF_VISIBLE = 32 * TF_DIRECTIONBASE;
	// public static final int TF_DISCOVERED = TF_VISIBLE;
	...
	public static int getMoveCost(final int t) {
		return movecost[t];
	}
	 ...
	 * @return
	 */
	public static int fromName(final String tileName) {
		if (nameById == null) {
			createMaps();
		}
		final Integer tile = (Integer) tileByName.get(tileName);
		if (tile != null) {
	...
	public static String tileNameFor(final int tile) {
		if (tileByName == null) {
			createMaps();
		}
		final String name = (String) nameById.get(new Integer(tile));
		if (name != null) {
			return name;
		}
	...
	public static int getImage(final int t) {
		return images[t];
	}
	...
	public static boolean isFilling(final int t) {
		return filling[t];
	}
	...
	private static void buildTileArrays() {
		final ArrayList al = Lib.instance().getTiles();
		final int n = al.size();
	...
	private static Thing get(final int t) {
		return tiles[t & 65535];
	}

	public static void action(final BattleMap m, final int x, final int y,
			final int time) {
		final int tile = m.getTile(x, y);
		final Thing t = get(tile);

		if (t.getFlag("IsDamageTile")) {
			final Thing[] ts = m.getThings(x, y);
			if (ts.length == 0) {
				return;
			}
	...
	private static boolean isDiggable(final Thing t) {
	...
	public static int digCost(final Thing b, final Thing w) {
		final int mining = b.getStat(Skill.MINING);
		if (b.getFlag("Digging")) {
			return 100;
		}
		if (mining <= 0) {
			return 0;
	...
	public static boolean dig(final Thing b, final BattleMap m, final int x,
			final int y) {
		final int tile = m.getTile(x, y);
		final Thing w = b.getWielded(RPG.WT_MAINHAND);
		final Thing t = get(tile);

		if (isDiggable(m, x, y)) {
			final int cost = digCost(b, w);
			if (cost <= 0) {
				return false;
			}
			final int hard = t.getStat("DigDifficulty");
			if (RPG.r(hard) < digAbility()) {
				if (w != null) {
					Damage.inflict(w, w.getStat("DigDamage"), RPG.DT_SPECIAL);
				}
				b.incStat("APS", -cost);
	...
	public static boolean dig(final BattleMap m, final int x, final int y) {
		if (!isDiggable(m, x, y)) {
			return false;
		}
		final Thing t = get(m.getTile(x, y));
		m.setTile(x, y, t.getStat("DigTile"));
		return true;
	}

	private static void addTile(int tile, final Thing t) {
		...
		int tileMask = 0;
		if (!t.getFlag("IsViewBlocking")) {
			tileMask |= Tile.TF_TRANSPARENT;
		}
		if (t.getFlag("IsBlocking")) {
			tileMask |= Tile.TF_BLOCKED;
		}
		if (t.getFlag("IsActive")) {
			tileMask |= Tile.TF_ACTIVE;
		}
	...
	public static int getMask(final int tile) {
	...
	public static int getMapColour(final int tile) {
	...
	private static void initLava() {
		final Thing t = Lib.extend("lava", "base water tile");
		t.set("Image", 47);
		t.set("IsBlocking", 0);
		t.set("IsLavaTile", 1);
		t.set("IsActive", 1);
