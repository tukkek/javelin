...
public abstract class PartyHazard extends Hazard{
	...
	/**
	 * Damages/penalizes a {@link Combatant} who failed it's save. Ideally should
	 * never outright kill a character since it's no fun to lose an unit passively
	 * (non-lethal damage). Also makes it a bit easier to code, without having to
	 * worry about {@link Ressurect}ions and such.
	 *
	 * @param c Penalizes/damages this character.
	...
	public void hazard(int hoursellapsed){
		...
		for(Combatant c:Squad.active.members)
			if(!save(hoursellapsed,c)) output+=affect(c,hoursellapsed)+", ";
		if(!output.isEmpty())
			Javelin.message(output.substring(0,output.length()-2)+"!",false);
	}
