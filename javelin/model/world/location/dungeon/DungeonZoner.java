...
public class DungeonZoner{
  ...
  public class Zone{
    ...
    void expand(){
      ...
      for(var zone:newzones) zone.expand();
    }
  }
  ...
  public DungeonZoner(DungeonFloor d,Point entrance){
    ...
    for(var zone:zones) for(var point:zone.area)
      for(var i=0;i<Math.pow(2,zone.depth);i++) result.add(point);
  }

  /**
   * @return A random point in the dungeon, with points in a deeper {@link Zone}
   *   being exponentially more likely to be chosen. Does not take into account
   *   dungeon size - so a shallow {@link Zone} that is very big will have more
   *   chance to be represented than a deep {@link Zone} that is very small.
   */
  ...
  /** Helper method to place a {@link Feature}. */
  public void place(Feature f,DungeonFloor floor){
    f.place(floor,getpoint());
  }
