...
public class MonsterGovernor extends Governor{
	...
	ArrayList<Labor> filter(Class<? extends Labor> cardtype,
		...
		ArrayList<Labor> found=new ArrayList<>();
		for(Labor l:hand)
			if(cardtype.isInstance(l)) found.add(l);
		return found;
	}

	@Override
	protected void always(ArrayList<Labor> hand){
		if(town.getrank().rank<=getseason()) start(filter(Growth.class,hand));
	}
}
